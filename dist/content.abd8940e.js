...
base("Work").select({
  ...
  record = records.filter(function (x) {
    return x.fields.Slug == searchParam;
  });
...
function createInterface(record, records) {
  ...
  var nextProjectSrcMob = document.getElementById("next-work-link-mob");
  var backProjectSrc = document.getElementById("back-work-link");
  var backProjectSrcMob = document.getElementById("back-work-link-mob");
  ...
  nextProjectBtnMob.addEventListener("click", function () {
    projectLocation = (projectLocation + 1) % records.length;
